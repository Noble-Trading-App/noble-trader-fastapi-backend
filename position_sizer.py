...
class PositionSizer:
    ...
    def size(self, req: PositionRequest) -> KellyResult:
        ...
        vol_scaled = float(np.clip(_vol_scale(frac_kelly, annual_sigma, req.target_vol), 0.0, 1.0))
        ...
        notes        = self._build_notes(full_kelly, req.kelly_fraction, annual_sigma,
                                         req.target_vol, regime_mult, sharpe)
    ...
    @staticmethod
    def _build_notes(full_kelly, kelly_fraction, annual_sigma,
                     target_vol, regime_mult, sharpe) -> list[str]:
        notes = []
        if full_kelly > 0.5:
...
class PositionSizeResult:
    ...
    def summary(self) -> str:
        ...
        return (
            ...
            + (f"  kelly_cap={self.kelly_cap:.4f}" if self.kelly_cap is not None else "")
        )
...
class DynamicMasanielloSizer:
    ...
    def quality_factor(self, p_win: float, regime_quality: float,
                       state_confidence: float) -> float:
        return _quality_factor(p_win, regime_quality, state_confidence,
    ...
    def size_trade(self, ctx: TradeContext) -> PositionSizeResult:
        ...
        # ── Compute factors ───────────────────────────────────────────────────
        M  = _masaniello_factor(ctx.wins_so_far, ctx.trade_index,
                                ctx.batch_size, ctx.target_wins)
        Q  = _quality_factor(ctx.p_win, ctx.regime_quality,
                             ctx.state_confidence, cfg.min_prob)
        ...
        if cfg.use_kelly_overlay:
            kelly_cap = _fractional_kelly(ctx.p_win, ctx.reward_risk,
                                          cfg.kelly_fraction)
            if kelly_cap > 0:
    ...
    def _blocked(reason: str) -> PositionSizeResult:
        return PositionSizeResult(
            allowed=False, risk_fraction=0.0, risk_amount=0.0,
            units=0.0, contracts=0,
            masaniello_factor=0.0, quality_factor=0.0,
            drawdown_factor=0.0, volatility_factor=0.0,
            expected_edge=0.0, kelly_cap=None, reason=reason,
        )
...
def monte_carlo_batch(
    ...
    for _ in range(n_simulations):
        ...
        for t in range(cfg.batch_size):
            ...
            ctx = TradeContext(**{
                k: v for k, v in base_context.__dict__.items()
            })
            ctx.trade_index      = t + 1
            ctx.wins_so_far      = wins
            ctx.losses_so_far    = t - wins
            ctx.current_drawdown = (
                max(0.0, (peak - (equity + pnl_total)) / peak)
                if peak > 0 else 0.0
            )
